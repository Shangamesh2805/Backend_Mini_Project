...
// Register services with Lazy<T> to avoid circular dependency
...
builder.Services.AddScoped(provider => new Lazy<IOrderServices>(() => provider.GetRequiredService<IOrderServices>()));
builder.Services.AddScoped(provider => new Lazy<IPaymentService>(() => provider.GetRequiredService<IPaymentService>()));
